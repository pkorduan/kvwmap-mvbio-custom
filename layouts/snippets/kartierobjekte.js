...
if ($('#105_bogenart_id_0').val() == 1) {
	console.log('hide beschreibung');
	//$('.105_group_FFH-LRT, .105_group_Beschreibung, .105_group_Standortmerkmale, .105_group_Nutzungsmerkmale, .105_group_Pflanzen').hide();
	$('.105_group_FFH-LRT, .105_group_Standortmerkmale, .105_group_Nutzungsmerkmale, .105_group_Pflanzen').hide();
}
...
function DatensatzValidator(layerId, datensatzNr) {
	...
	this.start = function () {
		...
		if (!label || label.length === 0) {
			...
			// Uneditierbar machen wenn kartierobjekt eine Bewertung hat
			if (document.getElementById(this.layerId + "_hat_bewertung_" + datensatzNr).value == 't') {
	...
	this.disableDropbox = function (elem) {
		...
		lblElem.innerText = elem.options[elem.selectedIndex].text;
